...
jQuery(document).ready(function () {
  ...
  jQuery(".prescription-btn").click(function () {
    ...
    var ajax_url =
      baseUrl +
      "/wp-content/themes/flatsome-child/ajax/update_lens_prescription.php";
    if (
      right_cyl !== "0.00" &&
      left_cyl !== "0.00" &&
      (right_axis === "0" || left_axis === "0")
    ) {
...
function selectLensType(lense_id, lense_name, product_id) {
  jQuery("#cover-spin").show(0);
  var ajax_url =
    baseUrl + "/wp-content/themes/flatsome-child/ajax/update_product_lens.php";
  jQuery.ajax({
    type: "post",
    url: ajax_url,
    data: {
...
function selectLensUsage(lens_id, lens_name) {
  jQuery("#cover-spin").show(0);
  var ajax_url =
    baseUrl + "/wp-content/themes/flatsome-child/ajax/update_lens_usage.php";
  jQuery.ajax({
    type: "post",
    url: ajax_url,
...
function addItemToCart(variation_id) {
  jQuery("#cover-spin").show(0);
  var ajax_url =
    baseUrl + "/wp-content/themes/flatsome-child/ajax/custom-add-to-cart.php";
  jQuery.ajax({
    type: "post",
    url: ajax_url,
...
function getConfirmationPopup(term_id, usage) {
  var ajax_url =
    baseUrl +
    "/wp-content/themes/flatsome-child/ajax/open-confirmation-popup.php";
...
jQuery("form.checkout.woocommerce-checkout").bind(
  "DOMSubtreeModified",
  function () {
    if (jQuery("ul.woocommerce-error").length) {
      jQuery("ul.woocommerce-error").insertAfter(".checkout-page-title"); //where you want to place it
    }
  }
);
