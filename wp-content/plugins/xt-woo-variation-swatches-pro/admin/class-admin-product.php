...
class XT_Woo_Variation_Swatches_Admin_Product {
    ...
    public function __construct( &$core ) {
        ...
        $this->core->plugin_loader()->add_action('admin_enqueue_scripts',  $this, 'enqueue_scripts' );

        $this->core->plugin_loader()->add_filter('woocommerce_product_data_tabs', $this, 'product_swatches_data_tab', 99 , 1 );
        $this->core->plugin_loader()->add_action('woocommerce_product_data_panels', $this, 'product_data_panel_wrap' );
        $this->core->plugin_loader()->add_action('woocommerce_process_product_meta', $this, 'process_meta_box', 1, 2 );
        $this->core->plugin_loader()->add_action('wp_ajax_xt_woovs_admin_tab_content', $this, 'load_tab_content_ajax');

        $this->core->plugin_loader()->add_action('save_post', $this, 'save_post_ajax');
        $this->core->plugin_loader()->add_action('admin_head-post.php', $this, 'enqueue_product_ajax_save_script');
        $this->core->plugin_loader()->add_action('admin_head-post-new.php', $this, 'enqueue_product_ajax_save_script');
                            ...
                            <tbody>
                                <tr>
                                    <td class="attribute_swatch_label">
                                        <strong>
                                            <span class="xt_woovs_edit_field row-title"><?php echo sanitize_text_field($current_label); ?></span>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td width="30" class="xt_woovs_preview_td <?php echo esc_attr($hide_preview);?>">
                                                    <div class="field_option field_option_color section-color-swatch <?php echo esc_attr($active_color);?>">
                                                        <div class="xt_woovs_preview xt_woovs_color_preview" <?php echo $color_preview; ?>></div>
                                                    </div>

                                                    <div class="field_option field_option_image xt_woovs_image_picker <?php echo esc_attr($active_image);?>">
                                                        <div class="xt_woovs_preview xt_woovs_image_preview" <?php echo $image_preview; ?>></div>
                                                    </div>
                                                </td>
    ...
    public function render_dropdown_field($field, $key_prefix, $key, $value) {
        ...
        <select class="xt_woovs_field" name="<?php echo esc_attr($key_prefix); ?>[<?php echo esc_attr($key);?>]">
            ...
            <?php foreach($options as $option_key => $option): ?>
                <option <?php selected( $value, $option_key ); ?> value="<?php echo esc_attr($option_key);?>"><?php echo sanitize_text_field($option);?></option>
            <?php endforeach; ?>
