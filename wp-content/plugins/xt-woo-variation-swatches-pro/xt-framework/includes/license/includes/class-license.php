...
class XT_Framework_License_3 {
	...
	public function __construct( &$manager, &$core ) {
		...
		if ( empty( $this->market_product()->id ) ) {
			...
			add_action( 'wp_ajax_' . $this->prefix( 'refresh_license_status' ), array( $this, 'ajax_refresh_license_status' ) );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( $this->core->framework_prefix('license'), $this->core->plugin_framework_url('/includes/license/assets/css', 'license.css'), array(), $this->core->framework_version(), 'all' );
		wp_register_script( $this->core->framework_prefix('license'), $this->core->plugin_framework_url('/includes/license/assets/js', 'license'.XTFW_SCRIPT_SUFFIX.'.js'), array( 'jquery' ), $this->core->framework_version(), false );
		...
		wp_localize_script( $this->core->framework_prefix('license'), 'XT_LICENSE_VARS', array(
		...
		wp_enqueue_script($this->core->framework_prefix('license'));
	}
	...
	public function market_product() {
		...
		return $this->core->market_product();
	}
	...
	public function form() {
		...
		?>
        <div id="xt-license-activation-<?php echo esc_attr( $this->market_product()->id ); ?>" class="xt-license-activation">
                ...
                <p class="xt-license-status">
                    ...
                    <span class="xt-license-submsg">
						...
						echo apply_filters(
							...
							sprintf(
								esc_html( 'Activate your %1$s to enable premium features, support & automated updates', 'xt-framework' ),
								$license_key_label
							),
							$this->market_product()
						);
            ...
            <div id="xt-license-revoke-form" class="xt-license-form xt-license-hide">
                ...
                <a href="<?php echo $this->market_product()->url; ?>" class="button" target="_blank"><?php echo esc_html__( 'Buy License', 'xt-framework' ); ?></a>
                <input type="submit" class="button button-primary" value="<?php echo esc_html__( 'Revoke', 'xt-framework' ); ?>">
            </div>
                    ...
                    <div class="xt-license-status-active<?php if ( ! $isActivated ): ?> xt-license-hide<?php endif; ?>">
                        ...
                        <span class="xt-license-submsg">
                            ...
                            echo apply_filters(
	                            ...
	                            sprintf(
		                            esc_html__( 'Support & automated updates are now %1$s', 'xt-framework' ),
                    ...
                    <div class="xt-license-status-expired<?php if ( $isActivated ): ?> xt-license-hide<?php endif; ?>">
                        ...
                        <span class="xt-license-submsg">
                            ...
                            echo apply_filters(
	                            ...
	                            sprintf(
		                            esc_html__( 'Support & automated updates are now %1$sDisabled%2$s!%3$sYou can still use the plugin%4$s and update it manually from Envato', 'xt-framework' ),
		                            "<strong>",
		                            "</strong>",
		                            "<br><strong>",
                                    "</strong>"
            ...
            <div id="xt-license-local-revoke-form" class="xt-license-form<?php if ( ! $isFound ): ?> xt-license-hide<?php endif; ?>">
                ...
                <a href="<?php echo $this->market_product()->url; ?>" class="button" target="_blank"><?php echo esc_html__( 'Buy License', 'xt-framework' ); ?></a>
                <input type="submit" class="button button-primary" value="<?php echo esc_html__( 'Revoke License', 'xt-framework' ); ?>">
            </div>
        </div>
	...
	public function remote_get( $url ) {
		...
		if ( is_wp_error( $response ) || ( ! empty( $response["response"]["code"] ) && $response["response"]["code"] === 403 ) ) {
			...
			if ( function_exists( 'curl_init' ) ) {
				...
				// If curl is not available try file_get_contents
				$response = @file_get_contents( $url );
