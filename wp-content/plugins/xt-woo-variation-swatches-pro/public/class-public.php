...
class XT_Woo_Variation_Swatches_Public {
	...
	public function __construct( &$core ) {
        ...
        $this->core->plugin_loader()->add_filter( 'woocommerce_dropdown_variation_attribute_options_html', $this, 'variation_attribute_options_html', 100, 2 );
        $this->core->plugin_loader()->add_filter( 'xt_woovs_swatch_html', $this, 'swatch_html', 5, 4 );
        $this->core->plugin_loader()->add_filter( 'xt_woovs_swatch_meta_html', $this, 'swatch_meta_html', 5, 4 );

        $this->core->plugin_loader()->add_action( 'init', $this, 'change_loop_add_to_cart', 10 );
	}
	...
	public function enqueue_styles() {
        ...
        wp_enqueue_style( $this->core->plugin_slug(), $this->core->plugin_url( 'public/assets/css', 'frontend.css'), array(), filemtime($this->core->plugin_path( 'public/assets/css', 'frontend.css')), 'all' );
	}
	...
	public function enqueue_scripts() {
        ...
        wp_register_script( $this->core->plugin_slug(), $this->core->plugin_url( 'public/assets/js', 'frontend'.XTFW_SCRIPT_SUFFIX.'.js'), array( 'jquery' ), filemtime($this->core->plugin_path( 'public/assets/js', 'frontend'.XTFW_SCRIPT_SUFFIX.'.js')), false );
            ...
            if($product && taxonomy_exists( $attribute ) ) {
                ...
                foreach ( $terms as $term ) {
					...
					if ( in_array( $term->slug, $options ) ) {
                        ...
                        $swatches .= apply_filters( 'xt_woovs_swatch_html', '', $term, $meta_values, $selected );
					}
				}
	 ...
	 * @return string
	 */
	public function swatch_html( $html, $term, $meta_values = array(), $selected = null) {
	...
	function loop_variation_add_to_cart_button() {
		...
		<div class="woocommerce-variation-add-to-cart xt_woovs-variation-add-to-cart variations_button">
			...
			echo apply_filters( 
                    ...
                    sprintf( '<button type="submit" class="single_add_to_cart_button button">%s</button>',
	...
	public function get_color_swatch_html($slug, $name, $color, $tooltip_data, $selected, $style) {
        ...
        if($this->core->access_manager()->can_use_premium_code__premium_only()) {
            ...
            if ($tooltip === 'image') {
                ...
                $tooltip_value = $tooltip_image ? wp_get_attachment_image_src($tooltip_image) : '';
                $tooltip_value = $tooltip_value ? $tooltip_value[0] : $this->core->plugin_url('admin/assets/images', 'placeholder.png');
	...
	public function get_image_swatch_html($slug, $name, $image, $tooltip_data, $selected, $style) {
        ...
        if($this->core->access_manager()->can_use_premium_code__premium_only()) {
            ...
            if ($tooltip === 'image') {
                ...
                $tooltip_value = $tooltip_image ? wp_get_attachment_image_src($tooltip_image) : '';
                $tooltip_value = $tooltip_value ? $tooltip_value[0] : $image;
                $tooltip_value = !empty($tooltip_value) ? $tooltip_value : $this->core->plugin_url('admin/assets/images', 'placeholder.png');
